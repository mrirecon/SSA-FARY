...
EOF = EOF / np.max(np.real(EOF)) * 3200 - 25
...
resp_interp_norm = - resp_interp / np.max(np.abs(resp_interp))  * 800 + 450
#%%
...
ax.plot(0.6 * np.real(-EOF[:,2]),  linewidth=2, color=color[0], label="EOF 3") # consider indexing
ax.plot(0.6 * np.real(-EOF[:,3]),  linewidth=2, color=color[4], label="EOF 4") # consider indexing
