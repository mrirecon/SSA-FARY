...
ax[0].grid()
ax[0].set_title("Respiration [a.u.] - W=51")
ax[0].set_ylim(-1.1,1.9)
ax[0].set_xlim(int_start,int_end) 
ax[0].set_xticklabels([])   

norm_r = np.max(np.abs(eof_51[:,1])[int_start:int_end])
ax[0].plot(t[int_start:int_end], np.real(eof_51[:,1])[int_start:int_end] / norm_r, label="EOF 2", linewidth=2, color=color[0])
ax[0].plot(t[int_start:int_end], np.real(eof_51[:,2])[int_start:int_end] / norm_r, label="EOF 3", linewidth=2, color=color[4])
ax[0].legend(ncol=3, loc='upper center', bbox_to_anchor=(0.5, 1.05))

norm_c = np.max(np.abs(eof_91[:,7])[int_start:int_end])
ax[1].grid()
ax[1].set_title("Cardiac [a.u.] - W=91")
ax[1].set_ylim(-1.1,1.9)
ax[1].set_xlim(int_start,int_end) 
ax[1].set_xlabel("Samples [a.u.]")
ax[1].plot(t[int_start:int_end], np.real(eof_91[:,7])[int_start:int_end] / norm_c, label="EOF 8", linewidth=2, color=color[4])
ax[1].plot(t[int_start:int_end], np.real(eof_91[:,8])[int_start:int_end] / norm_c, label="EOF 9", linewidth=2, color=color[0])
