...
#
# Script to reproduce Volunteer V4 of the following manuscript:
#
# Rosenzweig S et al.
...
#--- Traj ---
# topts="-x$RO -y$SP -t$FR -G -s7 -D -m$PAR -c"
# bart traj $topts _ta
...
bart reshape $(bart bitmask 2 3) 1 204 _kc1 _kc2
...
#--- Gating ---
python3 ../ssa_fary_utils/gating_analysis.py -s223 > Gating.txt

#--- Clean Up ---
rm pics_W${W}_Tr${Tr}_Tc${Tc}_Ts${Ts}_sc$scaling*.{cfl,hdr}
rm k*.{cfl,hdr} t*.{cfl,hdr} sens*.{cfl,hdr} rec1*.{cfl,hdr} eof.{cfl,hdr} pat*.{cfl,hdr} card*.{cfl,hdr}
