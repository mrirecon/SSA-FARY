...
#
# Script to reproduce Volunteer V2 of the following manuscript:
#
# Rosenzweig S et al.
...
rm ksg.{cfl,hdr} _ksg_cc*.{cfl,hdr}
...
rm card*.{cfl,hdr}
rm k*.{cfl,hdr} eof.{cfl,hdr} t*.{cfl,hdr} pat*.{cfl,hdr} sens*.{cfl,hdr} rec1*.{cfl,hdr}
