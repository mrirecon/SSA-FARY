...
GD=$(bart estdelay -R -r2 _tGD1 _kGD1); echo $GD
...
bart rtnlinv -s -A -d5 $GPU -t _tGD_rtos  _k_rt rec_rt

rm _*.{cfl,hdr}
rm t*.{cfl,hdr} pics_u*.{cfl,hdr} k*.{cfl,hdr} sens*.{cfl,hdr}
rm pat*.{cfl,hdr} eof*.{cfl,hdr} rec1*.{cfl,hdr}
