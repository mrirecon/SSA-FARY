...
#--- Traj ---
# topts="-x$RO -y$SP -t$FR -G -s7 -D -m$PAR -c -R-22"
# bart traj $topts _ta
...
for src in "k" "traj_cor" "eof"; do

	bart extract 10 0 860 $src __1
	bart extract 10 1180 2470 $src __2
	bart extract 10 2770 4840 $src __3
	bart extract 10 5170 5922 $src __4
	bart join 10 __{1,2,3,4} ${src}j
done

#--- Binning ---
R=9
C=25
bart bin -r0:1 -R$R -c2:3 -C$C -a300  -A30  eofj kj ksg
bart bin -r0:1 -R$R -c2:3 -C$C -a300  -A30  eofj traj_corj tsg
...
for ((i=0; i<$R; i+=1)); do
	...
	bart rtnlinv -s -A -x${dim}:${dim}:1 $GPU -m1 -d5 -t _tsg1os_crop _ksg1_crop _srec_r$i _ssens_r$i
...
rm _*.{cfl,hdr}
rm eof.{cfl,hdr} eofj.{cfl,hdr} rec1*.{cfl,hdr} t*.{cfl,hdr} k*.{cfl,hdr} 
rm pat*.{cfl,hdr} sens*.{cfl,hdr} card*.{cfl,hdr}
