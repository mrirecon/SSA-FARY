...
#
# Script to reconstruct Volunteer V1 of the following manuscript
#
# Rosenzweig S et al.
...
R=9
C=25
bart bin -r0:1 -R$R -c2:3 -C$C -a300 eof k ksg
bart bin -r0:1 -R$R -c2:3 -C$C -a300 -x card eof $DATA/traj_cor tsg
...
#--- Clean Up ---
rm pics_W${W}_Tr${Tr}_Tc${Tc}_Ts${Ts}_sc$scaling*
rm k*.{cfl,hdr} t*.{cfl,hdr} pat*.{cfl,hdr} eof.{cfl,hdr}
rm rec*.{cfl,hdr} sens*.{cfl,hdr} card*.{cfl,hdr}
