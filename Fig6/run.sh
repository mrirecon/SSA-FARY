...
#--- Traj ---
# topts="-x$RO -y$SP -t$FR -G -s7 -D -m$PAR -c -R-51"
# bart traj $topts _ta
...
bart reshape $(bart bitmask 2 3) 1 156 _kc1 _kc2
...
R=9
C=25
bart bin -r0:1 -R$R -c2:3 -C$C -a300 eof $DATA/traj_cor tsg
bart bin -r0:1 -R$R -c2:3 -C$C -a300 -x card eof k ksg
...
for ((i=0; i<$R; i+=1)); do
	...
	bart rtnlinv -x${dim}:${dim}:1 -m1 -s -A $GPU -d5 -t _tsg1os_crop _ksg1_crop _srec_r$i _ssens_r$i
...
for Ts in 0.0005; do
	...
	echo "W=" $W " Tr=" $Tr
	bart pics -M -d5 -m -i80 -p pat \
		-R W:$(bart bitmask 0 1):0:$W \
		-R T:$(bart bitmask 11):0:$Tr \
		-R T:$(bart bitmask 10):0:$Tc \
		-R T:$(bart bitmask 13):0:$Ts \
		-t tsg  ksg_sh sens pics_W${W}_Tr${Tr}_Tc${Tc}_Ts${Ts}_i80

done; done; done; done;

bart resize -c 0 192 1 192 pics_W${W}_Tr${Tr}_Tc${Tc}_Ts${Ts}_i80 pics_rs
...
#--- Clean up ---
rm pics_W${W}_Tr${Tr}_Tc${Tc}_Ts${Ts}_i80*.{cfl,hdr}
rm {sens,tsg,rec1,ksg_sh,pat,kc,eof,ac}.cfl 
rm {sens,tsg,rec1,ksg_sh,pat,kc,eof,ac}.hdr card*{cfl,hdr}
